...
acme_path=/opt/.acme.sh
mkdir -p $acme_path
docker stop nginx
docker stop trojan-go
check_domain
docker rm -f acme.sh
docker run --restart=always -itd  -v $acme_path/out:/acme.sh   --net=host  --name=acme.sh neilpang/acme.sh daemon
    if test -s $acme_path/out/server.crt; then
...
cat > $acme_path/renew_ssl.sh <<-EOF
docker stop nginx;
docker run --rm  -it -v /opt/.acme.sh/out:/acme.sh --net=host neilpang/acme.sh --cron > $acme_path/renew_ssl.history;
docker restart nginx
EOF
 ...
 x1=$[RANDOM%23+1]
 x2=$[RANDOM%59+1]
  (crontab -l|grep -v "$acme_path/renew_ssl.sh";echo "$x2 $x1  * * * bash $acme_path/renew_ssl.sh")| crontab
}
